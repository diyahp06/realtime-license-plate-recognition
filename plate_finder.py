...
def segment_chars(plate_img, fixed_width):
    ...
    if contours:
        ...
        for c in contours:
            ...
            if y > addPixel: y = y - addPixel
            else: y = 0
            if x > addPixel: x = x - addPixel
            else: x = 0
...
class PlateFinder:
    ...
    def ratioCheck(self, area, width, height):
        ratioMin, ratioMax = 3, 6
        ratio = float(width) / float(height)
        if ratio < 1: ratio = 1 / ratio
        if (area < self.min_area or area > self.max_area) or (ratio < ratioMin or ratio > ratioMax):
            return False
        return True
        
    def preRatioCheck(self, area, width, height):
        ratioMin, ratioMax = 2.5, 7
        ratio = float(width) / float(height)
        if ratio < 1: ratio = 1 / ratio
        if (area < self.min_area or area > self.max_area) or (ratio < ratioMin or ratio > ratioMax):
            return False
    ...
    def validateRatio(self, rect):
        ...
        if angle > 15 or height == 0 or width == 0: return False
        area = width * height
